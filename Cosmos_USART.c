...
{	
	...
	UCSR0B = (1<<RXCIE0)|(1<<RXEN0)|(1<<TXEN0);		//ATIVA TRANSMISSAO E RECEP��O E INTERRUP��O PARA RECEP��O
...
unsigned char SERIAL_RX_CHECK_ERROR()	//VERIFICA SE OCORREU ALGUM ERRO NA RECEP�AO DE DADOS
{										//PARA MAIORES DETALHES VERIFICAR REGISTRADOR 'UCSR0A' bits 2, 3 e 4.
	unsigned char trash;
	
	if(UCSR0A & 0b00011100)						//HOUVE ERRO DE RECEP�AO
	{
...
{
	while (!(UCSR0A & (1<<RXC0)));				//ESPERA A RECEP��O DE DADOS
