...
void EEPROM_Write(unsigned int address, unsigned char data)
{
	...
	EEAR = address;				//carrega o endereço para a escrita
...
{
	while(EECR & (1<<EEPE));	//espera completar um escrita prévia
	EEAR = address;				//escreve o endereço de leitura
...
{
	if (EEPROM_Read(address) != data)	//APENAS ATUALIZA EEPROM SE O ENTEREÇO CONTIVER UM DADO DIFERENTE
